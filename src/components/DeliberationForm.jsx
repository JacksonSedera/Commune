...
export default function DeliberationForm({ user = {}, setActiveView, deliberationToEdit }) {
  ...
  const convocationText = useMemo(() => {
    if (!form.date) return "";
    return `Le ${new Date(form.date).toLocaleDateString("fr-FR", {
      weekday: "long",
      day: "numeric",
      month: "long",
      year: "numeric",
    })} à ${form.heure} H, le Conseil municipal régulièrement convoqué par lettre du Président du Conseil Municipal n°${form.convocation} du ${form.dateConvocation}, s'est réuni en session extraordinaire sous la présidence de M. ${form.president}.`;
  }, [form.date, form.heure, form.convocation, form.dateConvocation, form.president]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.data.success) {
        await generateDeliberationPDF(form, repoblikaLogoDataUrl, communeLogoDataUrl);
  ...
  return (
    <div className="form-container">
      ...
      {showPreview && (
            ...
            <hr />
            <h4>Aperçu complet</h4>
            <div className="border p-3" style={{ whiteSpace: "pre-wrap" }}>
              <div style={{ textAlign: "center", fontWeight: "bold" }}>
                DELIBERATION N° {form.numero}-{(form.date ? new Date(form.date).getFullYear().toString().slice(-2) : "")} <br />
                {form.titreDeliberation}
              </div>

            <br />
            <strong>Conseillers présents :</strong>
            {(form.conseillersPresents || []).map((p, i) => (
              <div key={i}>MM. {p.nom}{p.fonction ? ` (${p.fonction})` : ""}</div>
            ))}

            <br />
            <strong>Conseillers représentés :</strong>
            {(form.representes || []).map((r, i) => (
              <div key={i}>{r.represente} représenté par {r.representant}</div>
            ))}
            ...
            <div style={{ fontWeight: "bold", textAlign: "center" }}>
              LE CONSEIL MUNICIPAL DE LA COMMUNE URBAINE DE MAHAJANGA
            </div>

            <div>-Vu la Constitution de la IVème République;</div>
            <div>-Vu la loi organique n°2014-018...</div>
            <div>-Vu la loi n°2014-020...</div>
            <div>-Vu la loi n°2014-021...</div>
            <div>-Vu le jugement n° 01/EL...</div>

            {(form.lignesJuridiques || []).map((line, i) => (
              <div key={i}>{line}</div>
            ))}

            <div>Entendu la présentation des membres de l'organe exécutif</div>

            <br />
            {(form.articles || []).map((a, i) => (
              <div key={i}>
                <strong>Article {i + 1} :</strong> {a}
              </div>
            ))}

            <br />
            <div style={{ display: "flex", justifyContent: "space-between", marginTop: "40px" }}>
              <div>Le Rapporteur<br />{form.rapporteur}</div>
              <div>Le Président<br />{form.presidentNom}</div>
            </div>
          </div>
        </div>
      )}
